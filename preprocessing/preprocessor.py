...
def tokenize_file(doc, tokenizer):
	final_sents = []
	for s in doc.sents:
		...
		for token in doc_t:
			...
			and token.text.lower() != 'y'    #no incluidos en los stopwords de Spacy
			and token.text.lower() != 'o' 
			and token.text.lower() != 'a' 
			and token.text.lower() != 'e'):
...
def save_lines(str, final_sents):
...
def preprocess_files():
	...
	for fname in os.listdir("../raw_texts"):
		doc = sentencize_file(fname)
		final_sents = tokenize_file(doc, tokenizer)
		save_lines(fname, final_sents)
